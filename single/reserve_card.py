...
def play(hand, is_start_of_round, play_to_beat, round_history, player_no, hand_sizes, scores, round_no, test_val):
    """
    ...
    * `round_no`: An integer between 0 and 9 (inclusive) indicating which round number is currently being played.
    ...
    if Card(card).value + min(hand_sizes) * 4 > test_val:
        return []
    else:
        return play_card
...
if __name__ == '__main__':
    ...
    TESTS = [  # [ expected return value, inputs ]
        [['AS'],
         [['4D', '4H', '7D', '8D', '8H', '0D', '0C', 'JH', 'QC', 'QS', 'KH', 'AS'], False, ['AC'], [[]], 0,
          [12, 8, 8, 8], [0, 0, 0, 0], 0]],
        # Add more tests here.
    ]
